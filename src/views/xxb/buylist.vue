      ...
      <div class="payfor">
        ...
        >
          <template>
            <div class="liexing">
              支付类型:
              <el-radio v-model="radio" label="微信" class="payType"
                >微信</el-radio
              >
              <el-radio v-model="radio" label="支付宝" class="payType"
                >支付宝</el-radio
              >
            </div>
          </template>
  ...
  methods: {
    fetchData() {
      this.$axios
        .post(
          this.$host +
            "/groupmall/power/mainList/pay?userId=" +
            myUserId +
            "&proId=" +
    ...
    onSubmit() {
      ...
      // console.log(YouhuiJuan.CouponId);
      ...
      this.$axios
        .post(
          this.$host +
            "/groupmall/power/mainList/newOrder?userId="+myUserId+"&proId="+this.$route.query.orderId+"&buyNum=1" +
            // "/groupmall/power/alipay/alipay?price=100.00&proName=apple" +
            "&payWay=" +
            this.radio +
            "&addressId=" +
            diZhi.id +
            "&couponId=" +
            (YouhuiJuan.CouponId === undefined ? "" : YouhuiJuan.CouponId) +
            "&note=" +
            this.pojo.message +
            "&finalPrice=" +
            this.endPrice
